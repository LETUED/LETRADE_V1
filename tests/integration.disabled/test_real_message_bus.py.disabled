    ...
    async def test_strategy_signal_real_flow(self, rabbitmq_config, strategy_config):
        ...
        try:
            ...
            if signal:
                ...
                assert success is True
                print(
                    f"✅ 신호 전송 성공: {payload['side']} @ {payload['signal_price']}"
                )
            else:
    ...
    async def test_end_to_end_signal_flow(self, rabbitmq_config):
        ...
        try:
            ...
            async def signal_handler(message_data):
                ...
                print(
                    f"💰 Capital Manager가 신호 수신: {payload.get('side')} {payload.get('symbol')} @ {payload.get('signal_price')}"
                )

            # Capital Manager 구독
            await subscriber_bus.subscribe(
                queue_name="capital_requests", callback=signal_handler, auto_ack=True
            )
            ...
            if signal:
                ...
                print(
                    f"🎉 E2E 테스트 성공: {received_signal['side']} 신호가 성공적으로 전달됨"
                )
            else:
            ...
            for i in range(max(4, len(historical_data) - 3), len(historical_data)):
                ...
                if signal:
                    ...
                    if success:
                        ...
                        print(
                            f"📈 백테스트 신호 전송: {payload['side']} @ {payload['signal_price']}"
                        )
