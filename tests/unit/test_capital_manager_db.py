...
class TestCapitalManagerDatabase:
    ...
    @patch("src.capital_manager.main.db_manager")
    def test_save_trade_to_db(
        self, mock_db_manager, capital_manager, sample_trade_request
    ):
        """거래 저장 테스트"""
        # Mock 설정
        mock_session = MagicMock()
    ...
    @patch("src.capital_manager.main.db_manager")
    @pytest.mark.asyncio
    async def test_validate_trade_with_db_integration(
        self, mock_db_manager, capital_manager, sample_trade_request
    ):
...
class TestCapitalManagerStateReconciliation:
    ...
    @patch("src.capital_manager.main.db_manager")
    @pytest.mark.asyncio
    async def test_full_state_reconciliation_flow(
        self, mock_db_manager, capital_manager
    ):
        """완전한 상태 조정 플로우 테스트"""
        # Mock 설정
        mock_session = MagicMock()
