...
def golden_cross_data():
    ...
    prices = [
        49000, 49100, 49200, 49300, 49400,  # 초기 상승
        49500, 49700, 50000, 50300, 50600,  # 가속 상승 (크로스오버 발생)
        50800, 51000, 51200, 51400, 51600   # 지속 상승
    ]
...
def death_cross_data():
    ...
    prices = [
        51000, 50900, 50800, 50700, 50600,  # 초기 하락
        50400, 50100, 49800, 49500, 49200,  # 가속 하락 (크로스오버 발생)
        49000, 48800, 48600, 48400, 48200   # 지속 하락
    ]
...
class TestMAcrossoverStrategy:
    ...
    def test_is_signal_allowed_old_signal(self, ma_strategy):
        """오래된 신호 후 허용 테스트"""
        ma_strategy._last_signal_timestamp = datetime.now(timezone.utc) - timedelta(seconds=120)
        assert ma_strategy._is_signal_allowed() is True
