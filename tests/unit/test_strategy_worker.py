...
class TestStrategyWorker:
    ...
    async def test_worker_start_failure_handling(self, strategy_worker):
        ...
        with patch('src.strategy_worker.main.ProcessPoolExecutor') as mock_executor_class:
    ...
    async def test_worker_restart(self, strategy_worker):
        ...
        with patch.object(strategy_worker, 'stop') as mock_stop:
            ...
            with patch.object(strategy_worker, 'start') as mock_start:
                ...
                assert result is True
                assert strategy_worker.metrics.restart_count == original_restart_count + 1
...
class TestStrategyWorkerManager:
    ...
    @pytest.mark.asyncio
    async def test_add_duplicate_worker(self, worker_manager, strategy_config, worker_config):
        """중복 Worker 추가 테스트"""
        await worker_manager.add_worker("test_001", MockStrategy, strategy_config, worker_config)
        
        # 같은 ID로 다시 추가 시도
        result = await worker_manager.add_worker("test_001", MockStrategy, strategy_config, worker_config)
    ...
    @pytest.mark.asyncio
    async def test_start_all_workers(self, worker_manager, strategy_config, worker_config):
        """모든 Worker 시작 테스트"""
        # 여러 Worker 추가
        await worker_manager.add_worker("test_001", MockStrategy, strategy_config, worker_config)
        await worker_manager.add_worker("test_002", MockStrategy, strategy_config, worker_config)
        
        with patch.object(worker_manager.workers["test_001"], 'start') as mock_start1:
            mock_start1.return_value = True
            with patch.object(worker_manager.workers["test_002"], 'start') as mock_start2:
    ...
    @pytest.mark.asyncio
    async def test_stop_all_workers(self, worker_manager, strategy_config, worker_config):
        """모든 Worker 중지 테스트"""
        await worker_manager.add_worker("test_001", MockStrategy, strategy_config, worker_config)
        await worker_manager.add_worker("test_002", MockStrategy, strategy_config, worker_config)
    ...
    @pytest.mark.asyncio
    async def test_get_all_health_status(self, worker_manager, strategy_config, worker_config):
...
@pytest.mark.skipif(
    ...
    reason="Process tests require spawn start method"
)
