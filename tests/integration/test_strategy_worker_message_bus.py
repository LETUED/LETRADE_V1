...
from src.strategy_worker.main import StrategyWorker, StrategyWorkerManager, WorkerConfig, WorkerStatus
...
class TestStrategyWorkerMessageBusIntegration:
    ...
    @pytest.mark.asyncio
    async def test_worker_message_bus_connection(self, strategy_config, worker_config, mock_message_bus):
        """Worker가 메시지 버스에 정상적으로 연결되는지 테스트"""
        
        with patch('src.strategy_worker.main.MessageBus', return_value=mock_message_bus):
    ...
    @pytest.mark.asyncio
    async def test_market_data_subscription(self, strategy_config, worker_config, mock_message_bus):
    ...
    async def test_signal_routing_format(self, strategy_config):
        ...
        required_fields = ["strategy_id", "symbol", "side", "signal_price", "confidence"]
        for field in required_fields:
            assert field in payload
    
    @pytest.mark.asyncio
    async def test_worker_manager_integration(self, strategy_config, worker_config, mock_message_bus):
...
class TestMessageFlowEnd2End:
    ...
    @pytest.mark.asyncio 
    async def test_error_handling_in_message_flow(self, strategy_config, mock_message_bus):
        """메시지 흐름에서 에러 처리 테스트"""
