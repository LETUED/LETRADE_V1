...
class TestMessageBusIntegration:
    ...
    async def test_core_engine_message_bus_integration(self):
        ...
        with patch("common.message_bus.aio_pika.connect_robust", return_value=AsyncMock()):
            with patch.object(MessageBus, "_setup_infrastructure", return_value=None):
                ...
                with patch.object(engine, "_validate_startup_conditions", return_value=True):
                    with patch.object(engine, "_start_background_tasks", return_value=None):
                        with patch.object(engine, "_perform_startup_reconciliation", return_value=None):
                ...
                with patch.object(engine, "_stop_new_operations", return_value=None):
                    with patch.object(engine, "_wait_for_active_operations", return_value=None):
                        with patch.object(engine, "_stop_background_tasks", return_value=None):
                            with patch.object(engine, "_persist_final_state", return_value=None):
                                await engine.stop()
    ...
    async def test_message_flow_system_events(self):
        ...
        with patch("common.message_bus.aio_pika.connect_robust", return_value=AsyncMock()):
            with patch.object(MessageBus, "_setup_infrastructure", return_value=None):
                ...
                assert "test_strategy" not in engine.status.active_strategies  # Stopped at the end
        ...
        with patch("common.message_bus.aio_pika.connect_robust", return_value=AsyncMock()):
            with patch.object(MessageBus, "_setup_infrastructure", return_value=None):
                ...
                result = await publisher.publish(
                    "letrade.requests",
                    MessageRoutes.CAPITAL_ALLOCATION.format(strategy_id="ma_crossover_1"),
                    test_message
                )
    ...
    async def test_message_persistence_and_durability(self):
        ...
        with patch("common.message_bus.aio_pika.connect_robust", return_value=AsyncMock()):
            with patch.object(MessageBus, "_setup_infrastructure", return_value=None):
                ...
                message_arg = call_args[0][0]  # First positional argument (Message object)
    ...
    async def test_dead_letter_queue_setup(self):
        ...
        with patch("common.message_bus.aio_pika.connect_robust", return_value=AsyncMock()):
            ...
            async def mock_declare_queue(name, exchange_name, routing_key, durable=True, **kwargs):
                # Verify DLX configuration in queue arguments
                if 'arguments' in kwargs:
                    assert 'x-dead-letter-exchange' in kwargs['arguments']
                    assert kwargs['arguments']['x-dead-letter-exchange'] == 'letrade.dlx'
    ...
    async def test_core_engine_startup_performance_with_message_bus(self):
        ...
        with patch("common.message_bus.aio_pika.connect_robust", return_value=AsyncMock()):
            with patch.object(MessageBus, "_setup_infrastructure", return_value=None):
                ...
                assert startup_duration < 2.0, f"Startup took {startup_duration:.3f}s, should be < 2.0s"
