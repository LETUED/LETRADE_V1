...
class TestRealMessageBusIntegration:
    ...
    async def real_message_bus(self, rabbitmq_config):
        ...
        for attempt in range(max_retries):
            if await message_bus.connect():
                ...
                break
            logger.warning(f"RabbitMQ 연결 실패, 재시도 중... ({attempt + 1}/{max_retries})")
            await asyncio.sleep(1)
        else:
            pytest.skip("RabbitMQ 서버에 연결할 수 없습니다. docker-compose up -d 를 실행하세요.")
    ...
    async def test_real_core_engine_message_bus_integration(self, rabbitmq_config):
        ...
        assert "test_strategy_1" not in engine.status.active_strategies  # 마지막에 정지됨
    ...
    async def test_real_high_throughput_messaging(self, real_message_bus):
        ...
        assert publish_throughput > 50, f"발행 처리량이 너무 낮음: {publish_throughput:.1f} msg/s"
        assert received_count >= message_count * 0.9, f"메시지 손실: {received_count}/{message_count}"
...
class TestRealMessageBusPerformance:
    ...
    async def test_real_connection_performance(self, performance_message_bus):
        ...
        # 연결 시간이 2초 미만이어야 함
        assert avg_connection_time < 2.0, f"연결 시간이 너무 느림: {avg_connection_time:.3f}s"
