...
class TestRefactoredCapitalManager:
    ...
    def capital_manager(self, config):
        ...
        manager.db_handler.get_portfolio_metrics = AsyncMock(
            return_value=PortfolioMetrics(
                total_value=10000.0,
    ...
    async def test_validate_trade_emergency_stop(self, capital_manager):
        ...
        # 거래 요청
        trade_request = TradeRequest(
            strategy_id=1, symbol="BTC/USDT", side="buy", quantity=0.01, price=50000.0
        )
    ...
    async def test_validate_trade_daily_loss_limit(self, capital_manager):
        ...
        # 포트폴리오 메트릭스 업데이트
        capital_manager.db_handler.get_portfolio_metrics = AsyncMock(
            return_value=PortfolioMetrics(
                total_value=10000.0,
                available_cash=5000.0,
                unrealized_pnl=0.0,
    ...
    async def test_validate_trade_blocked_symbol(self, capital_manager):
        ...
        # 차단된 심볼 거래 요청
        trade_request = TradeRequest(
            strategy_id=1, symbol="LUNA/USDT", side="buy", quantity=10.0, price=1.0
        )
    ...
    async def test_portfolio_impact_calculation(self, capital_manager):
        ...
        assert response.portfolio_impact["position_size_percent"] == pytest.approx(
            5.0
        )  # 500/10000
    ...
    async def test_trade_execution_recording(self, capital_manager):
        ...
        # 기록
        result = await capital_manager.record_trade_execution(
...
class TestValidationRules:
    ...
    async def test_rule_enable_disable(self):
        ...
        # 비활성화된 규칙 찾기
        emergency_rule = next(
            (r for r in engine.rules if r.name == "EmergencyStop"), None
        )
