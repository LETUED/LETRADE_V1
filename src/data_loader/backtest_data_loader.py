...
class BaseDataLoader(ABC):
    ...
    def normalize_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.debug(f"Data normalized: {len(df)} rows, {df.index.min()} to {df.index.max()}")
...
class MockDataLoader(BaseDataLoader):
    ...
    async def load_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Generating {len(timestamps)} mock data points for {self.config.symbol}")
...
class BacktestDataLoader:
    ...
    def get_data_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "volume_stats": {
                "avg": float(data['volume'].mean()) if 'volume' in data.columns else None,
                "total": float(data['volume'].sum()) if 'volume' in data.columns else None
            }
        }
