...
class MAcrossoverStrategy(BaseStrategy):
    ...
    def __init__(self, config: StrategyConfig):
        ...
        # 신호 강도 설정
        self.min_crossover_strength = config.custom_params.get('min_crossover_strength', 0.001)  # 0.1%
    ...
    def populate_indicators(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.debug(
                ...
                extra={
                    "strategy_id": self.strategy_id,
                    "fast_ma_last": df['ma_fast'].iloc[-1] if not df['ma_fast'].isna().all() else None,
                    "slow_ma_last": df['ma_slow'].iloc[-1] if not df['ma_slow'].isna().all() else None,
                    "signal_last": df['ma_signal'].iloc[-1] if not df['ma_signal'].isna().all() else None
                }
            )
    ...
    def get_required_subscriptions(self) -> List[str]:
        ...
        """
        routing_key = f"market_data.{self.exchange}.{self.symbol.lower().replace('/', '')}"
        ...
        return {
            ...
            "current_state": {
                ...
                "last_signal_time": (
                    ...
                    if self._last_signal_timestamp else None
                )
            }
        }
