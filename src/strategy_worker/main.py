        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if self.process and not self.process.is_alive():
                    ...
                    # 자동 재시작
                    if (
                        self.worker_config.enable_auto_restart
                        and self.metrics.restart_count
...
class StrategyProcessRunner:
    ...
    async def _start_strategy(self):
        ...
        if not await self.strategy.start():
            raise Exception(
                f"Failed to start strategy {self.strategy_config.strategy_id}"
            )
    ...
    async def _send_trading_signal(self, signal):
        ...
        signal_routing_key = signal.get(
            "routing_key",
            f"request.capital.allocation.{self.strategy_config.strategy_id}",
        )
...
class StrategyWorkerManager:
    ...
    def get_running_worker_count(self) -> int:
        """실행 중인 Worker 개수 반환"""
        return len(
            [w for w in self.workers.values() if w.get_status() == WorkerStatus.RUNNING]
        )
