...
class Portfolio(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "total_capital > 0", name="ck_portfolios_total_capital_positive"
        ),
        CheckConstraint(
            "available_capital >= 0",
            name="ck_portfolios_available_capital_non_negative",
        ),
...
class Strategy(Base):
    ...
    position_sizing_config = Column(
        JSONB, default={"model": "fixed_fractional", "risk_percent": 0.02}
    )
    ...
    performance_metrics = relationship(
        "PerformanceMetric", back_populates="strategy", cascade="all, delete-orphan"
    )
...
class Position(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "avg_entry_price > 0", name="ck_positions_avg_entry_price_positive"
        ),
        CheckConstraint(
            "stop_loss_price > 0 OR stop_loss_price IS NULL",
            name="ck_positions_stop_loss_positive",
        ),
        CheckConstraint(
            "take_profit_price > 0 OR take_profit_price IS NULL",
            name="ck_positions_take_profit_positive",
        ),
