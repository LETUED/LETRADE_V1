...
class DatabaseInitializer:
    ...
    def _create_initial_data(self) -> None:
        ...
        with self._get_session() as session:
            try:
                # 기본 포트폴리오가 존재하는지 확인
                existing_portfolio = session.query(Portfolio).filter_by(name="Default Portfolio").first()
    ...
    def _create_default_portfolio_rules(self, session: Session, portfolio_id: int) -> None:
        """기본 포트폴리오 리스크 규칙 생성"""
        default_rules = [
            {
...
class DatabaseHealthChecker:
    ...
    def check_tables(self) -> Dict[str, Any]:
        """필수 테이블 존재 여부 확인"""
        required_tables = [
            'portfolios', 'strategies', 'trades', 
            'positions', 'portfolio_rules', 'performance_metrics',
            'system_logs'
        ]
