...
class OptimizedRepository:
    ...
    async def get_portfolio_metrics(self, portfolio_id: int) -> Dict[str, Any]:
        ...
        ).where(and_(Position.portfolio_id == portfolio_id, Position.is_open == True))
        ...
        today_pnl_query = select(func.sum(Trade.realized_pnl)).where(
            and_(
    ...
    async def get_active_strategies(self, portfolio_id: int) -> List[Strategy]:
        ...
        query = (
            select(Strategy)
            .where(
                and_(Strategy.portfolio_id == portfolio_id, Strategy.is_active == True)
            )
            .options(selectinload(Strategy.trades))  # 관련 거래 미리 로드
        )
        ...
        try:
            ...
            for trade in trades:
                cache_key = self.cache_manager.cache_key(
                    "portfolio_metrics", trade.portfolio_id
                )
    ...
    async def get_recent_trades(
        ...
        # 캐시 키
        cache_key = self.cache_manager.cache_key(
            "recent_trades", portfolio_id, limit=limit, offset=offset
        )
    ...
    async def log_system_event_batch(self, events: List[Dict[str, Any]]) -> bool:
        """시스템 이벤트 배치 로깅"""
        try:
            logs = [
                SystemLog(
