...
class ReconciliationReport:
    ...
    def get_critical_discrepancies(self) -> List[StateDiscrepancy]:
        ...
        return [
            ...
            if d.severity
            in [ReconciliationSeverity.HIGH, ReconciliationSeverity.CRITICAL]
        ]
...
class StateReconciliationEngine:
    ...
    def __init__(self, exchange_connector, capital_manager):
        ...
        self.max_position_variance = Decimal(
            "0.001"
    ...
    async def perform_full_reconciliation(self) -> ReconciliationReport:
        ...
        self.current_report = ReconciliationReport(
            session_id=session_id, start_time=datetime.now(timezone.utc)
        )
            ...
            with db_manager.get_session() as session:
                ...
                for portfolio in portfolios:
                    ...
                    if portfolio_currency in exchange_balances:
                        ...
                        variance = abs(system_total - exchange_total) / max(
                            system_total, Decimal("1")
                        )
                        ...
                        discrepancy = StateDiscrepancy(
                            ...
                            exchange_state={
                                "available_currencies": list(exchange_balances.keys())
                            },
        ...
        try:
            ...
            with db_manager.get_session() as session:
                ...
                for db_position in db_positions:
                    ...
                    exchange_balances = (
                        await self.exchange_connector.get_account_balance()
                    )
                    ...
                    if symbol_base in exchange_balances:
                        ...
                        if hasattr(db_position, "side") and db_position.side == "long":
                            ...
                            variance = abs(expected_holding - actual_holding) / max(
                                expected_holding, Decimal("0.001")
                            )
                        ...
                        self.current_report.strategies_checked.add(
                            db_position.strategy_id
                        )
    ...
    async def _reconcile_strategy_states(self) -> None:
        ...
        try:
            with db_manager.get_session() as session:
                active_strategies = (
                    session.query(Strategy).filter_by(is_active=True).all()
                )
    ...
    async def _apply_auto_corrections(self) -> None:
        ...
        auto_correctable = [
            d
            for d in self.current_report.discrepancies
    ...
    async def _apply_correction(
        self, discrepancy: StateDiscrepancy
    ...
    async def _finalize_reconciliation(self) -> None:
        ...
        if hasattr(self.exchange_connector, "config") and hasattr(
            self.exchange_connector.config, "exchange_name"
        ):
            self.current_report.exchanges_checked.add(
                self.exchange_connector.config.exchange_name
            )
        else:
    ...
    async def _save_reconciliation_report(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Reconciliation report saved (session: {self.current_report.session_id})"
            )
