...
class Portfolio(Base):
    ...
    rules = relationship(
        "PortfolioRule", back_populates="portfolio", cascade="all, delete-orphan"
    )
    strategies = relationship(
        "Strategy", secondary="strategy_portfolio_map", back_populates="portfolios"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "total_capital >= 0", name="ck_portfolios_positive_total_capital"
        ),
        CheckConstraint(
            "available_capital >= 0", name="ck_portfolios_positive_available_capital"
        ),
        CheckConstraint(
            "available_capital <= total_capital",
            name="ck_portfolios_available_le_total",
        ),
    )
...
class Strategy(Base):
    ...
    portfolios = relationship(
        "Portfolio", secondary="strategy_portfolio_map", back_populates="strategies"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "name", "exchange", "symbol", name="uq_strategies_name_exchange_symbol"
        ),
...
class Position(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "position_type IN ('spot', 'futures', 'margin')",
            name="ck_positions_valid_type",
        ),
        CheckConstraint(
            "leverage >= 1 AND leverage <= 125", name="ck_positions_valid_leverage"
        ),
    )
...
class StakingLog(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "transaction_type IN ('stake', 'unstake', 'claim_rewards')",
...
class GridOrder(Base):
    ...
    amount = Column(DECIMAL(20, 8), nullable=False)
    exchange_order_id = Column(
        String(255), ForeignKey("trades.exchange_order_id"), nullable=True
    )
