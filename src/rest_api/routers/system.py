...
from ..auth.dependencies import require_system_read, require_system_write, require_admin
...
async def execute_system_command(
    ...
    try:
        ...
        logger.info(f"User {current_user.username} executing system command: {command_data.command}")
        ...
        result_message = command_results.get(
            ...
            f"Command {command_data.command} executed"
        )
        ...
        logger.info(f"System command {command_data.command} executed successfully by {current_user.username}")
    ...
    try:
        ...
        system_info = {
            ...
            "components": {
                ...
                "message_bus": {
                    "status": "connected" if service_status["message_bus_connected"] else "disconnected",
...
async def exit_maintenance_mode(
    ...
    try:
        ...
        return MessageResponse(
            ...
            timestamp="2024-06-24T05:41:00Z",
            data={
                "maintenance_ended_by": current_user.username
            }
        )
    ...
    try:
        ...
        if response.get("success", True):
            return MessageResponse(
                ...
                timestamp="2024-06-24T05:41:00Z",
                data={
                    "session_id": response.get("session_id", "manual-" + str(id(reconciliation_request))),
        ...
        mock_users = [
            {
                ...
                "username": "admin",
                "permissions": [
                    "system:read", "system:write", "strategies:read", 
                    "strategies:write", "strategies:control", "portfolios:read", 
                    "portfolios:write", "monitoring:read"
                ],
                "active": True,
            ...
            },
            {
                ...
                "username": "trader",
                "permissions": [
                    "strategies:read", "strategies:control", 
                    "portfolios:read", "monitoring:read"
                ],
                "active": True,
                "last_login": None,
                "created_at": "2024-06-01T00:00:00Z"
            },
            {
                ...
                "username": "viewer",
                "permissions": [
                    "strategies:read", "portfolios:read", "monitoring:read"
                ],
                "active": True,
                "last_login": None,
                "created_at": "2024-06-01T00:00:00Z"
