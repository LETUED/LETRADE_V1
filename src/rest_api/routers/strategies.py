...
from ..auth.dependencies import (
    require_strategies_control,
    require_strategies_read,
    require_strategies_write,
)
...
async def list_strategies(
    status_filter: Optional[StrategyStatus] = Query(
...
async def create_strategy(
    ...
    try:
        response = await api_service.create_strategy(
            current_user.user_id, strategy_data.model_dump()
        )
        ...
        logger.info(
            f"Created strategy {strategy_result.get('id')} for user {current_user.username}"
        )
...
async def control_strategy(
    ...
    try:
        ...
        logger.info(
            f"Strategy {strategy_id} {control_data.action} by user {current_user.username}"
        )
