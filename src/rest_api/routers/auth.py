...
from ..schemas.responses import MessageResponse, TokenResponse, UserResponse
...
async def login(
    login_data: LoginRequest, jwt_handler: JWTAuthHandler = Depends()
) -> TokenResponse:
...
async def refresh_token(
    token_data: TokenRequest, jwt_handler: JWTAuthHandler = Depends()
) -> TokenResponse:
    """Refresh JWT token.
...
async def logout(
    ...
    return MessageResponse(
        message="Successfully logged out", timestamp="2024-06-24T05:41:00Z"
    )
