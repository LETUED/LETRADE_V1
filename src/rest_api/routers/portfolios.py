...
async def list_portfolios(
    ...
    """
    try:
        filters = {"active_only": active_only, "limit": limit, "offset": offset}
...
async def create_portfolio(
    ...
    try:
        ...
        logger.info(
            f"Created portfolio {mock_portfolio['id']} for user {current_user.username}"
        )
...
async def update_portfolio(
    ...
    try:
        ...
        logger.info(
            f"Updated portfolio {portfolio_id} for user {current_user.username}"
        )
...
async def get_portfolio_metrics(
    portfolio_id: int,
    period: str = Query(
        "daily", pattern="^(daily|weekly|monthly)$", description="Metrics period"
    ),
    current_user: UserPayload = Depends(require_portfolios_read),
    api_service: APIService = Depends(),
) -> PortfolioMetricsResponse:
