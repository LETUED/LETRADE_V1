...
class PortfolioResponse(BaseModel):
    ...
    available_capital: Decimal = Field(description="Available capital for new positions")
...
class PortfolioUpdateRequest(BaseModel):
    ...
    description: Optional[str] = Field(default=None, max_length=500, description="Portfolio description")
    
    # Risk limits updates
    risk_limits: Optional[Dict[str, Any]] = Field(default=None, description="Risk limits configuration")
    config: Optional[Dict[str, Any]] = Field(default=None, description="Portfolio configuration")
...
class PortfolioCreateRequest(BaseModel):
    ...
    def validate_name(cls, v):
        if not v.replace('_', '').replace('-', '').replace(' ', '').isalnum():
            raise ValueError('Portfolio name must contain only alphanumeric characters, spaces, hyphens, and underscores')
        return v
...
class PortfolioMetricsResponse(BaseModel):
    ...
    downside_volatility: Optional[float] = Field(default=None, description="Downside volatility")
    max_drawdown: Optional[float] = Field(default=None, description="Maximum drawdown")
    var_95: Optional[Decimal] = Field(default=None, description="Value at Risk (95%)")
    cvar_95: Optional[Decimal] = Field(default=None, description="Conditional VaR (95%)")
