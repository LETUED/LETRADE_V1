...
class StrategyCreateRequest(BaseModel):
    ...
    risk_config: Optional[Dict[str, Any]] = Field(
        default=None, description="Risk management configuration"
    )
    ...
    max_position_size: Optional[Decimal] = Field(
        default=None, gt=0, description="Maximum position size"
    )
    ...
    def validate_name(cls, v):
        if not v.replace("_", "").replace("-", "").replace(" ", "").isalnum():
            raise ValueError(
                "Strategy name must contain only alphanumeric characters, spaces, hyphens, and underscores"
            )
        return v
    ...
    def validate_config(cls, v, values):
        ...
        if strategy_type == StrategyType.MA_CROSSOVER:
            ...
            for field in required_fields:
                if field not in v:
                    raise ValueError(
                        f"MA Crossover strategy requires {field} in config"
                    )
        return v
...
class StrategyUpdateRequest(BaseModel):
    ...
    name: Optional[str] = Field(
        default=None, min_length=3, max_length=100, description="Strategy name"
    )
    description: Optional[str] = Field(
        default=None, max_length=500, description="Strategy description"
    )
    ...
    config: Optional[Dict[str, Any]] = Field(
        default=None, description="Strategy configuration parameters"
    )
    risk_config: Optional[Dict[str, Any]] = Field(
        default=None, description="Risk management configuration"
    )
    ...
    allocated_capital: Optional[Decimal] = Field(
        default=None, gt=0, description="Capital allocation"
    )
    max_position_size: Optional[Decimal] = Field(
        default=None, gt=0, description="Maximum position size"
    )
...
class StrategyPerformanceResponse(BaseModel):
    ...
    avg_win: Optional[Decimal] = Field(
        default=None, description="Average winning trade"
    )
    avg_loss: Optional[Decimal] = Field(
