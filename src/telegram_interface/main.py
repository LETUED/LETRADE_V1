...
class TelegramBot:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.message_integration = TelegramMessageIntegration(
            self.command_handler, self.notification_manager
        )
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            await self.notification_manager.start(
                self.application.bot, self.message_bus
            )

            # Start message integration
            await self.message_integration.connect(
                self.message_bus, self.application.bot
            )
    ...
    async def _register_handlers(self) -> None:
        ...
        self.application.add_handler(TgCommandHandler("start", self._handle_start))
        self.application.add_handler(TgCommandHandler("stop", self._handle_stop))
        self.application.add_handler(TgCommandHandler("restart", self._handle_restart))

        # Information commands
        self.application.add_handler(TgCommandHandler("help", self._handle_help))
        self.application.add_handler(TgCommandHandler("status", self._handle_status))
        self.application.add_handler(
            TgCommandHandler("portfolio", self._handle_portfolio)
        )
        self.application.add_handler(TgCommandHandler("report", self._handle_report))
        ...
        self.application.add_handler(TgCommandHandler("menu", self._handle_main_menu))
    ...
    async def _handle_legacy_positions(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> None:
    ...
    async def _handle_legacy_strategies(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> None:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if (
            hasattr(self.command_handler, "hourly_reporter")
            and self.command_handler.hourly_reporter
        ):
        ...
        return {
            ...
            "hourly_reporting_enabled": getattr(
                self.command_handler, "reporting_enabled", False
            ),
