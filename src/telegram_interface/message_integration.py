...
class TelegramMessageIntegration:
    ...
    def __init__(self, command_handler: CommandHandler, notification_manager: NotificationManager):
    ...
    async def _setup_notification_subscriptions(self) -> None:
        ...
        if hasattr(self.notification_manager, 'message_bus'):
            logger.info("Notification subscriptions already configured in NotificationManager")
        else:
            logger.warning("NotificationManager not properly configured with message bus")
    ...
    async def _handle_strategy_control_response(self, message: Dict[str, Any]) -> None:
        ...
        try:
            logger.debug(f"Received strategy control response: {message.get('request_id')}")
    ...
    async def send_command(self, routing_key: str, command_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(f"Sent command via {routing_key}: {command_data.get('request_id')}")
            return True
