...
class ServiceClient:
    ...
    async def get_portfolio_status(self) -> Dict[str, Any]:
        ...
        try:
            # Try REST API first
            if self.session:
                async with self.session.get(f"{self.api_base_url}/api/portfolio") as response:
                    if response.status == 200:
                        return await response.json()
    ...
    async def get_positions_status(self) -> Dict[str, Any]:
        ...
        try:
            # Try REST API first
            if self.session:
                async with self.session.get(f"{self.api_base_url}/api/positions") as response:
                    if response.status == 200:
                        return await response.json()
    ...
    async def get_strategies_status(self) -> Dict[str, Any]:
        ...
        try:
            # Try REST API first
            if self.session:
                async with self.session.get(f"{self.api_base_url}/api/strategies") as response:
                    if response.status == 200:
                        return await response.json()
    ...
    async def start_trading_system(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.session:
                ...
                async with self.session.post(
                    f"{self.api_base_url}/api/system/start",
                    json=payload
                ) as response:
                    if response.status == 200:
                        return await response.json()
    ...
    async def stop_trading_system(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.session:
                ...
                async with self.session.post(
                    f"{self.api_base_url}/api/system/stop",
                    json=payload
                ) as response:
                    if response.status == 200:
                        return await response.json()
    ...
    async def _send_strategy_command(self, action: str, strategy_id: int, user_id: int) -> Dict[str, Any]:
