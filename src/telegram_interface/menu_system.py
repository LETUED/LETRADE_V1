...
class MenuSystem:
    ...
    def _format_main_menu_text(self, system_status: Dict) -> str:
        ...
        trading_status = "🟢 활성" if system_status.get('trading_active', False) else "🔴 중지"
...
💡 **팁**: 각 설정은 즉시 적용되며, 안전을 위해 중요한 변경사항은 확인 절차를 거칩니다.
    ...
    async def show_trading_settings(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("🎯 전략 활성화/비활성화", callback_data="trading_strategies")],
            
            # Risk level adjustment
            [InlineKeyboardButton("⚖️ 리스크 레벨 조정", callback_data="trading_risk_level")],
...
설정을 변경할 항목을 선택하세요:
        ...
        self._update_user_session(user_id, MenuLevel.TRADING, {'config': trading_config})
    
    async def show_portfolio_settings(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        keyboard = [
            # Asset allocation
            [InlineKeyboardButton("📊 자산 배분 설정", callback_data="portfolio_allocation")],
            
            # Rebalancing
            [InlineKeyboardButton("⚖️ 리밸런싱 설정", callback_data="portfolio_rebalancing")],
            
            # Profit taking rules
            [InlineKeyboardButton("💰 수익실현 규칙", callback_data="portfolio_profit_taking")],
            
            # Stop loss settings
            [InlineKeyboardButton("🛑 손절매 설정", callback_data="portfolio_stop_loss")],
            
            # Diversification rules
            [InlineKeyboardButton("🎯 분산투자 규칙", callback_data="portfolio_diversification")],
...
관리할 항목을 선택하세요:
        ...
        self._update_user_session(user_id, MenuLevel.PORTFOLIO, {'config': portfolio_config})
    
    async def show_confirmation_dialog(self, update: Update, action: str, details: Dict) -> None:
...
⚠️ **주의**: 이 작업은 실제 거래에 영향을 줄 수 있습니다.
    ...
    async def _handle_menu_navigation(self, update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str) -> None:
    ...
    async def _handle_settings_navigation(self, update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str) -> None:
    ...
    async def _handle_action_request(self, update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str) -> None:
    ...
    async def _handle_quick_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str) -> None:
...
**응답시간**: {status.get('avg_response_time', 0):.1f}ms
            ...
            keyboard = [[InlineKeyboardButton("🔄 새로고침", callback_data="quick_status"),
    ...
    async def _show_notifications_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Show notifications settings menu."""
        keyboard = [
            [InlineKeyboardButton("📱 일반 알림 설정", callback_data="notify_general")],
            [InlineKeyboardButton("🚨 긴급 알림 설정", callback_data="notify_emergency")],
    ...
    async def _show_security_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async def _show_advanced_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async def _execute_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE, action: str) -> None:
    ...
    async def _handle_confirmation(self, update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str) -> None:
