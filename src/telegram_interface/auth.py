...
class AuthManager:
    ...
    def __init__(self, config: Dict):
        ...
        logger.info(
            f"AuthManager initialized with {len(self.allowed_user_ids)} allowed user IDs "
    ...
    def _is_authorized(self, user_id: int, username: Optional[str] = None) -> bool:
        ...
        if self.require_username and not username:
            logger.warning(
                f"User {user_id} denied access: username required but not provided"
            )
            return False
    ...
    async def _record_failed_attempt(self, user_id: int, reason: str) -> None:
        ...
        logger.warning(
            f"Failed authentication attempt {failed_data['count']} for user {user_id}: {reason}"
        )

        # Log security alert if approaching lockout
        if failed_data["count"] >= self.max_failed_attempts - 1:
            logger.error(
                f"SECURITY ALERT: User {user_id} approaching lockout threshold"
            )
    ...
    def verify_security_token(self, provided_token: str) -> bool:
        ...
        expected_token = hmac.new(
            self.security_token.encode(), b"letrade_auth", hashlib.sha256
        ).hexdigest()
    ...
    def get_auth_stats(self) -> Dict:
        ...
        active_sessions = len(
            [
                s
                for s in self.authenticated_users.values()
