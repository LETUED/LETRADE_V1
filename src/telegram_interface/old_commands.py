...
💡 **다음 조치:** {'정상 운영 중' if abs(daily_pnl) < 3 else '주의 깊은 모니터링 필요'}"""
    ...
    async def handle_positions(
        ...
        try:
            ...
            await message_bus.publish(
                routing_key="request.positions.status", message=positions_request
            )
    ...
    async def handle_strategies(
        ...
        try:
            ...
            # Send request via message bus
            await message_bus.publish(
                routing_key="request.strategies.status", message=strategies_request
            )
    ...
    async def handle_stop_strategy(
        ...
        try:
            ...
            result = await self.service_client.stop_strategy(
                strategy_id, update.effective_user.id
            )
    ...
    async def handle_start_strategy(
        self,
        update: Update,
        context: ContextTypes.DEFAULT_TYPE,
        message_bus: MessageBus,
        ...
        try:
            ...
            await message_bus.publish(
                routing_key="commands.strategy.start", message=start_command
            )
    ...
    async def handle_profit(
        ...
        try:
            ...
            # Send request via message bus
            await message_bus.publish(
                routing_key="request.profit.analysis", message=profit_request
            )

            period_korean = {"today": "오늘", "week": "이번 주", "month": "이번 달"}
    ...
    async def process_response(
        self, response_data: Dict[str, Any], bot: Optional[Any] = None
    ) -> None:
        ...
        try:
            ...
            message = await self._format_response(
                request_type, response_data, pending_request
            )
            ...
            if bot:
                ...
                logger.warning(
                    f"Cannot send response - bot instance not provided for request {request_id}"
                )
    ...
    async def _format_response(
        ...
        if request_type == "status":
            ...
            return self._format_strategy_control_response(
...
🕐 마지막 업데이트: {datetime.now(timezone.utc).strftime('%H:%M:%S UTC')}
    ...
    def _format_strategy_control_response(
        self, response_data: Dict[str, Any], pending_request: Dict[str, Any]
    ) -> str:
