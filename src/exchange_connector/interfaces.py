...
class OrderResponse:
    ...
    def from_ccxt_order(cls, ccxt_order: Dict[str, Any]) -> 'OrderResponse':
        ...
        return cls(
            ...
            average_price=Decimal(str(ccxt_order['average'])) if ccxt_order.get('average') else None,
            cost=Decimal(str(ccxt_order['cost'])) if ccxt_order.get('cost') else None,
            fee=Decimal(str(ccxt_order['fee']['cost'])) if ccxt_order.get('fee') else None,
            timestamp=datetime.fromtimestamp(ccxt_order['timestamp'] / 1000, tz=timezone.utc)
        )
...
class AccountBalance:
    ...
    @classmethod
    def from_ccxt_balance(cls, currency: str, balance_data: Dict[str, Any]) -> 'AccountBalance':
...
class IExchangeConnector(ABC):
    ...
    @abstractmethod
    async def get_market_data(self, symbol: str, timeframe: str = '1m', limit: int = 100) -> List[MarketData]:
    ...
    @abstractmethod
    async def subscribe_market_data(self, symbols: List[str], callback: Callable[[MarketData], None]) -> bool:
