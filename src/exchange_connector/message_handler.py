...
class ExchangeMessageHandler:
    ...
    async def _handle_trade_command(self, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            order_request = OrderRequest(
                ...
                stop_loss=(
                    Decimal(str(message["stop_loss"]))
                    if message.get("stop_loss")
                    else None
                ),
                take_profit=(
                    Decimal(str(message["take_profit"]))
                    if message.get("take_profit")
                    else None
                ),
            ...
            execution_event = {
                ...
                "average_price": (
                    float(order_response.average_price)
            ...
            await self.message_bus.publish(
                routing_key=self.response_keys["error_event"], message=error_event
            )
    ...
    async def start_market_data_stream(self, symbols: List[str]) -> bool:
        ...
        try:
            ...
            success = await self.exchange.subscribe_market_data(
                symbols, market_data_callback
            )
    ...
    async def _publish_market_data_event(self, market_data) -> None:
        ...
        try:
            ...
            await self.message_bus.publish(
                routing_key=routing_key, message=market_data_event
            )
