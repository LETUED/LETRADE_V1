...
class CCXTExchangeConnector(IExchangeConnector):
    ...
    async def subscribe_market_data(
        ...
        try:
            ...
            if not hasattr(self.exchange, "watch_ohlcv"):
                logger.warning(
                    f"{self.config.exchange_name} does not support WebSocket streaming"
                )
                return False
    ...
    async def get_account_balance(self) -> Dict[str, AccountBalance]:
        ...
        try:
            ...
            for currency, balance_data in ccxt_balance.items():
                if currency not in ["info", "free", "used", "total"] and isinstance(
                    balance_data, dict
                ):
                    balances[currency] = AccountBalance.from_ccxt_balance(
                        currency, balance_data
                    )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            if self.is_connected:
                ...
                health_status.update(
                    {"status": "disconnected", "api_accessible": False}
                )
            ...
            health_status.update(
                {"status": "unhealthy", "error": str(e), "api_accessible": False}
            )
    ...
    async def _monitor_symbol_stream(
        self, symbol: str, callback: Callable[[NewMarketData], None]
    ) -> None:
            ...
            while symbol in self._market_subscriptions and self.is_connected:
                try:
                    ...
                    if ohlcv_data:
                        ...
                        market_data = NewMarketData.from_ccxt_ohlcv(
    ...
    async def _handle_exchange_error(self, error: Exception) -> None:
        ...
        logger.error(
            f"Exchange error (failure {self._circuit_breaker_failures}): {error}"
        )
...
def create_exchange_connector(
    exchange_name: str, config: Dict[str, Any], use_ccxt: bool = False
) -> BaseExchangeConnector:
    ...
    if use_ccxt and HAS_CCXT:
        ...
        if use_ccxt and not HAS_CCXT:
            logger.warning(
                "CCXT requested but not available, falling back to Mock connector"
            )
