...
from .interfaces import TradeRequest, RiskParameters, RiskLevel
...
class DailyLossLimitRule(ValidationRule):
    ...
    async def validate(self, trade_request: TradeRequest, context: Dict[str, Any]) -> bool:
        ...
        potential_loss_percent = daily_loss_percent + (estimated_risk / portfolio_metrics.total_value) * 100
...
class PositionLimitRule(ValidationRule):
    ...
    async def validate(self, trade_request: TradeRequest, context: Dict[str, Any]) -> bool:
        ...
        # 심볼별 포지션 수 확인
        symbol_positions = sum(1 for p in positions.values() if p.get('symbol') == trade_request.symbol)
