...
from .interfaces import (
    TradeRequest, ValidationResponse, PortfolioMetrics,
    PositionInfo, TradeExecution, RiskParameters
)
...
class CapitalManagerDatabaseHandler:
    ...
    async def load_portfolio_rules(self) -> Dict[str, Any]:
        ...
        try:
            with self.get_db_session() as session:
                rules = session.query(PortfolioRule).filter_by(
                    portfolio_id=self.portfolio_id,
                    is_active=True
                ).all()
        ...
        try:
            with self.get_db_session() as session:
                trade = Trade(
                    ...
                    status="validated" if validation_response.is_approved() else "rejected",
                    validation_result={
    ...
    async def log_risk_event(
        ...
        try:
            with self.get_db_session() as session:
                ...
                logger.warning(
                    f"Risk event logged: {event_type}",
        ...
        try:
            with self.get_db_session() as session:
                ...
                trade = session.query(Trade).filter_by(
                    id=int(trade_execution.trade_id)
                ).first()
