...
class RefactoredCapitalManager:
    ...
    async def _prepare_validation_context(
        ...
        current_price = trade_request.price or await self._get_current_price(
    ...
    def _create_approval_response(
        ...
        if not trade_request.stop_loss:
            response.suggested_stop_loss = self._calculate_stop_loss(
                trade_request, context
            )

        # Take Profit 제안
        if not trade_request.take_profit:
            response.suggested_take_profit = self._calculate_take_profit(
                trade_request, context
            )

        # 포트폴리오 영향 계산
        response.portfolio_impact = self._calculate_portfolio_impact(
            trade_request, context
        )
    ...
    def _calculate_estimated_risk(
        self, trade_request: TradeRequest, current_price: float
    ) -> float:
        """예상 리스크 계산"""
        if current_price <= 0:
            return 0.0
    ...
    def _calculate_portfolio_impact(
        ...
        return {
            ...
            "available_capital_after": portfolio_metrics.available_cash
            - position_value,
        }
    ...
    async def _get_current_price(self, symbol: str) -> float:
        ...
        # 임시로 기본값 반환
        default_prices = {"BTC/USDT": 50000.0, "ETH/USDT": 3000.0, "BNB/USDT": 400.0}
        return default_prices.get(symbol, 100.0)

    async def _record_validation_result(
        self, trade_request: TradeRequest, response: ValidationResponse
    ) -> None:
        """검증 결과 기록"""
        try:
    ...
    async def emergency_stop(self, reason: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"Emergency stop activated", extra={"reason": reason})
