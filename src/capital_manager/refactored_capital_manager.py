...
class ValidationRule:
    ...
    def get_rejection_reason(
        self, request: TradeRequest, context: Dict[str, Any]
    ) -> str:
        """거부 사유 반환"""
        raise NotImplementedError
...
class DailyLossLimitRule(ValidationRule):
    ...
    async def validate(self, request: TradeRequest, context: Dict[str, Any]) -> bool:
        ...
        daily_loss_percent = (
            abs(portfolio_metrics.realized_pnl_today / portfolio_metrics.total_value)
            * 100
        )
...
class PositionLimitRule(ValidationRule):
    ...
    async def validate(self, request: TradeRequest, context: Dict[str, Any]) -> bool:
        ...
        symbol_positions = sum(
            1 for p in positions.values() if p.get("symbol") == request.symbol
        )
...
class RefactoredCapitalManager:
    ...
    async def validate_trade(self, trade_request: TradeRequest) -> ValidationResponse:
        ...
        response = self._create_validation_response(
            trade_request, context, validation_errors, risk_level
        )
...
async def example_usage():
    ...
    # 거래 요청
    trade_request = TradeRequest(
        strategy_id=1, symbol="BTC/USDT", side="buy", quantity=0.1, price=50000.0
    )
